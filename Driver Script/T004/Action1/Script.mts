'Test Objective : Import LO csv files in Oracle and SQL environment
Option explicit
...
Dim vBL_App_Path,vBL_App_Setting_Path,vTP_Path ,vTP,vTCD4,vTPS,vTSEC, vUsername,vPassword
...
Environment.LoadFromFile("C:\Automation\BL_iEx\Xml_File\BX_TP01_T004.xml")
...
vCSVPath = Trim(fnReadFromExcel(vTP_Path,vTCD4,2,2))
...
If  fnReadFromExcel(vTP_Path,vTPS,21,2) = "Yes" and fnReadFromExcel(vTP_Path,vTPS,22,2) = "Yes" Then
		Do while   fnReadFromExcel(vTP_Path,vTCD4,vRowControl,3) <> ""
								If    fnReadFromExcel(vTP_Path,vTCD4,vRowControl,3) = "Yes" and _
									   fnReadFromExcel(vTP_Path,vTSEC,9,5) = "Yes" then
										...
										Case else
												print "BX_TP01_T004 - Invalid number Case Number- Test Halts"
										End Select
									End If
...
'''****************** For Oracle Environment************************
		        If  fnReadFromExcel(vTP_Path,vTPS,21,2) = "Yes" and fnReadFromExcel(vTP_Path,vTPS,22,2) = "No" and fnReadFromExcel(vTP_Path,vTCD4,3,3) = "Yes" and _
											fnReadFromExcel(vTP_Path,vTSEC,9,5) = "Yes" Then
											On error resume next
											fnT001 vRowControl,vReplaceImportFolderLine
				End If
...
'***************
'''****************** For SQL Environment************************
If  fnReadFromExcel(vTP_Path,vTPS,21,2) = "No" and fnReadFromExcel(vTP_Path,vTPS,22,2) = "Yes" and  fnReadFromExcel(vTP_Path,vTCD4,4,3) = "Yes" and _
											fnReadFromExcel(vTP_Path,vTSEC,9,5) = "Yes" Then
											On error resume next
...
SystemUtil.Run "C:\Automation\BL_iEx\Batch Files\T004_LO_FRC_CreateFolderCopyFileIntoIt.bat"
'''**************************
Function fnT001(vRowControl,vReplaceImportFolderLine)
					 fnBluelightSettingToORACLE(vBL_App_Setting_Path)
					vReport1 =Trim(fnReadFromExcel(vTP_Path,vTCD4,vRowControl,2)) ' "C:\Automation\Bluelight iExchange\Local_CSVFolder\T003\Report3A_SQL"
					...
					If  vSymOraLogon.exist(1) Then
								fnOracleLogOn vUsername, vPassword
								 fnImportProcessToDisplayReport4 vCSVPath
...
'				*********************
				If Trim(OptionalStep.SwfWindow("Symphony Import").SwfObject("Welcome to the Symphony").GetROProperty("Text")) = vWelcomePage Then
						...
						print "Welcome page found in LO"
				End If
				...
				if (instr(OptionalStep.SwfWindow("Symphony Import").SwfObject("IgnoreFailLOFRC").GetROProperty("Text"), "last") <> 0) then 
				
						wait(3)
						SwfWindow("Symphony Import").SwfObject("grpProcess").Click 254,35
						wait(2)
						SwfWindow("Symphony Import").SwfObject("Next >").Click 36,11
						print "Got ignore"
				end if
